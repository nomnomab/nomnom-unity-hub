...
pub fn extract_template_information(app: &tauri::AppHandle, app_state: &tauri::State<AppState>, surface_template: &SurfaceTemplate) -> Result<TgzPackageJsonRecord, errors::AnyError> {
  ...
  if let Some(package_json_contents) = package_json_contents {
    ...
    if let Some(package_lock_json_contents) = package_lock_json_contents {
      ...
      let mut dep_children = HashMap::new();
      ...
      if !package_lock_json_contents.is_empty() {
        ...
        for (key, value) in package_lock_json.dependencies {
          if !value.version.starts_with("file:") && !found_deps.contains_key(&key) {
            found_deps.insert(key.clone(), value.version);
          }
          
          for (subkey, subvalue) in value.dependencies {
            dep_children.insert(key.clone(), subkey.clone());
      ...
      for (key, value) in found_deps {
        // check if this key is in any children
        if dep_children.iter().any(|x| x.1 == &key) {
          continue;
        }

        final_deps.insert(key, value);
      }
